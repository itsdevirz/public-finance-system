...
function SField({ value, onChange, options, disabled }) {
  return (
    <select value={value} onChange={onChange} disabled={disabled}
      className={cn("rounded-lg border border-input bg-background px-3 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-ring/30 focus:border-primary transition-all duration-200", disabled && "opacity-50 cursor-not-allowed")}>
          ...
          {activeTab === "basic" && (
            <div className="space-y-4">
              ...
              <div className="form-grid">
                ...
                <Field label="نوع مال" required><SField value={form.assetType} onChange={set("assetType")} options={ASSET_TYPES} /></Field>
                <Field label="ماهیت مال"><SField value={form.assetNature} onChange={set("assetNature")} options={ASSET_NATURES} /></Field>
                ...
                <Field label="واحد"><SField value={form.unit} onChange={set("unit")} options={UNITS} /></Field>
                <Field label="تامین‌کننده"><Input value={form.supplier} onChange={set("supplier")} className="h-8 text-sm" /></Field>
                <Field label="وضعیت"><SField value={form.status} onChange={set("status")} options={STATUSES} /></Field>
              </div>
            </div>
          )}
              ...
              {isConsumable ? (
                ...
                <>
                  ...
                  <div className="form-grid">
                    ...
                    <Field label="وضعیت برچسب"><SField value={form.labelStatus} onChange={set("labelStatus")} options={[{value:"active",label:"فعال"},{value:"damaged",label:"آسیب‌دیده"},{value:"replaced",label:"تعویض شده"}]} /></Field>
                  </div>
                </>
              )}
            </div>
          ...
          {activeTab === "location" && (
            <div className="space-y-5">
              ...
              <div className="form-grid">
                ...
                <Field label="وضعیت تحویل"><SField value={form.deliveryStatus} onChange={set("deliveryStatus")} options={[{value:"delivered",label:"تحویل داده شده"},{value:"returned",label:"عودت داده شده"},{value:"pending",label:"در انتظار"}]} /></Field>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              {isConsumable ? (
                ...
                <>
                  ...
                  <div className="form-grid">
                    <Field label="روش استهلاک"><SField value={form.depreciationMethod} onChange={set("depreciationMethod")} options={DEPREC_METHODS} /></Field>
