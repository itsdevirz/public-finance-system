...
export const BUDGET_CHAPTERS_LIST = [
...
export const INITIAL_FORM_10P_ROWS = BUDGET_CHAPTERS_LIST.map((chapterTitle, index) => ({
  id: `10p_${index + 1}`,
  creditType: "مصوب",
...
export default function SanamaForm10PChapterUnconsumed({
  rows = INITIAL_FORM_10P_ROWS,
  onChange,
  moeinBalancesMap = {},
  onSyncMoein,
  fiscalYear = "1404",
  ...
  const [formRows, setFormRows] = useState(rows && rows.length > 0 ? rows : INITIAL_FORM_10P_ROWS);
  ...
  // افزودن سطر فصل جدید
  const handleAddRow = () => {
    const newId = `10p_${Date.now()}`;
    const newRow = {
      id: newId,
      creditType: "مصوب",
      chapterTitle: `فصل سفارشی ${formRows.length + 1}`,
      notifier: "-",
      initialYearBalance: 0,
      transferredFromNonDefinite: 0,
      transferredFromObjectionsAndDeficit: 0,
  ...
  // محاسبات مجموع سرجمع فصول و محاسباتی‌ها بر اساس قوانین فرم ۱۰-پ سناما
  ...
  return (
    <div className="space-y-4 text-right select-none dir-rtl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
        ...
        <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-right">
          <div className="flex items-center justify-between text-[11px] font-bold text-emerald-700 dark:text-emerald-400">
            <span>مانده پایان سال (فصلی)</span>
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 p-2.5 rounded-xl bg-muted/40 border border-border/70">
        <div className="flex items-center gap-2">
          <Badge variant="default" className="text-xs font-bold px-2.5 py-1 gap-1">
            ...
            <span>تفکیک فصول: {toPersianDigits(formRows.length)} فصل بودجه‌ای</span>
          </Badge>
          <span className="text-xs text-muted-foreground hidden sm:inline">
            صورت فصلی وجوه مصرف نشده و انتقالی اعتبارات (فرم ۱۰ - پ فصلی)
          </span>
        </div>
                  ...
                  >
                    ...
                    <td className="p-1.5">
                      <Input
                        ...
                        placeholder="فصل اعتبار بودجه‌ای..."
                      />
                    </td>
                ...
                <tr className="bg-primary/10 text-primary-foreground font-bold text-xs border-t-2 border-primary/40 divide-x divide-x-reverse divide-primary/20">
                  <td colSpan={4} className="p-3 text-right text-foreground font-bold">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>سرجمع کل فصول وجوه مصرف نشده ({toPersianDigits(formRows.length)} فصل):</span>
                    </div>
                  </td>
      ...
      {/* ─── راهنمای کدهای معین و قوانین سناما فرم ۱۰-پ ─── */}
