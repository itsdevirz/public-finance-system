...
import { Save, Info, CheckCircle2, Pencil, Trash2, Plus, X } from "lucide-react";
  ...
  return (
    <div className="grid grid-cols-2 gap-x-8 gap-y-4" dir="rtl">
      <Field label="نوع هزینه" required>
        <SearchableSelect
          value={data.expenseKind}
          onChange={v => set("expenseKind", v)}
          options={EXPENSE_KIND_OPTIONS.filter(o => o.value !== "")}
      ...
      <Field label="فصول" required>
        <SearchableSelect
          value={data.expenseChapter}
          onChange={v => set("expenseChapter", v)}
          options={EXPENSE_CHAPTER_OPTIONS.filter(o => o.value !== "")}
        ...
        <SearchableSelect
          ...
          options={
            data.expenseChapter && SUB_CHAPTER_MAP[data.expenseChapter]?.length
              ? SUB_CHAPTER_MAP[data.expenseChapter]
              : ALL_SUB_CHAPTERS.filter(o => o.value !== "" && !o.value.startsWith("_")).map(o => ({
                  ...o,
                  group: (() => {
                    const headers = ALL_SUB_CHAPTERS.filter(x => x.value.startsWith("_"));
                    for (let i = headers.length - 1; i >= 0; i--) {
                      const hIdx = ALL_SUB_CHAPTERS.indexOf(headers[i]);
                      const oIdx = ALL_SUB_CHAPTERS.indexOf(o);
                      if (oIdx > hIdx) return headers[i].label.replace(/─/g, "").trim();
                    }
                    return "";
                  })(),
                }))
          }
          placeholder="انتخاب ریزفصل"
        />
      </Field>
    </div>
  );
...
function CapitalFields({ data, onChange }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-x-8 gap-y-4" dir="rtl">
      ...
      <Field label="فصول" required>
        <SearchableSelect
          value={data.capitalChapter}
          onChange={v => set("capitalChapter", v)}
          options={EXPENSE_CHAPTER_OPTIONS.filter(o => o.value !== "")}
          placeholder="انتخاب فصل"
        />
      </Field>
            ...
            <Field label="نوع اعتبار" required>
              <div className="max-w-xs">
                <SearchableSelect
                  ...
                  onChange={v => setForm(f => ({ ...f, creditType: v }))}
                  options={CREDIT_TYPE_OPTIONS.filter(o => o.value !== "")}
                  placeholder="انتخاب نوع اعتبار"
                  searchable={false}
                />
              </div>
            </Field>
