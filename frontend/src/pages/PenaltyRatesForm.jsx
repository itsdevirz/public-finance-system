...
const PENALTY_NATURES = [
  { value: "تأخیر در تعهدات", label: "تأخیر در تعهدات" },
  { value: "کیفیت کار", label: "کیفیت کار" },
  { value: "تخلفات قراردادی", label: "تخلفات قراردادی" },
  { value: "سایر", label: "سایر" },
];

const CALC_METHODS = [
  { value: "روزانه درصدی", label: "روزانه درصدی" },
  { value: "روزانه مبلغ ثابت", label: "روزانه مبلغ ثابت" },
  { value: "کل درصدی", label: "کل درصدی (مقطوع)" },
  { value: "کل مبلغ ثابت", label: "کل مبلغ ثابت (مقطوع)" },
];

const INITIAL_FORM = {
  code: "",
  title: "",
  nature: "تأخیر در تعهدات",
  calcMethod: "روزانه درصدی",
  rate: 0,
  amount: 0,
  status: "فعال",
  description: "",
...
export default function PenaltyRatesForm() {
  ...
  const fetchList = async () => {
    setLoading(true);
    try {
      const res = await api.get("/api/penalty-rates");
      if (res.data?.success) {
        setList(res.data.data || []);
      }
    } catch (err) {
      console.error("Error fetching penalty rates:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getSuggestedCode = async () => {
    try {
      const res = await api.get("/api/penalty-rates/suggest-code");
  ...
  const handleSave = async (e) => {
    ...
    if (!form.code || !form.title.trim()) {
      alert("لطفاً تمامی فیلدهای الزامی (کد و عنوان جریمه) را پر کنید.");
      return;
    }

    try {
      const isPercent = form.calcMethod.includes("درصدی");
      const payload = {
        ...form,
        rate: isPercent ? (Number(form.rate) || 0) : 0,
        amount: !isPercent ? (Number(form.amount) || 0) : 0,
      };

      if (selectedId) {
        // Edit mode
        const res = await api.put(`/api/penalty-rates/${selectedId}`, payload);
        if (res.data?.success) {
          setSaved(true);
          fetchList();
        ...
        // Create mode
        const res = await api.post("/api/penalty-rates", payload);
        if (res.data?.success) {
          setSaved(true);
          fetchList();
          handleNew();
      ...
      console.error("Error saving penalty rate:", err);
  ...
  const handleDelete = async () => {
    ...
    if (!confirm("آیا از حذف این نرخ جریمه مطمئن هستید؟")) return;

    try {
      const res = await api.delete(`/api/penalty-rates/${selectedId}`);
      if (res.data?.success) {
        alert("نرخ جریمه با موفقیت حذف شد.");
        fetchList();
        handleNew();
      }
    } catch (err) {
      console.error("Error deleting penalty rate:", err);
      alert(err.response?.data?.message || "خطا در حذف نرخ جریمه.");
    }
  };
  ...
  const handleRowClick = (item) => {
    ...
    setForm({
      code: item.code || "",
      title: item.title || "",
      nature: item.nature || "تأخیر در تعهدات",
      calcMethod: item.calcMethod || "روزانه درصدی",
      rate: item.rate || 0,
  ...
  const isPercentMethod = form.calcMethod.includes("درصدی");
    ...
    <PageShell>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-border/80 pb-4 mb-5" dir="rtl">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-lg font-bold text-foreground">تعریف نرخ جرایم</h1>
            <p className="text-xs text-muted-foreground">تعریف و مدیریت آیین‌نامه‌ها و نرخ‌های محاسباتی جرایم دیرکرد، تخلفات پیمانکاری و کیفیت کار</p>
          </div>
        </div>
          ...
          <Card className="border-border/80 shadow-sm">
            ...
            <CardContent className="pt-5 pb-6">
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <Field label="کد نرخ جریمه" required>
                ...
                <Field label="عنوان جریمه" required>
                  <Input
                    type="text"
                    value={form.title}
                    onChange={(e) => setForm(prev => ({ ...prev, title: e.target.value }))}
                    placeholder="مثال: جریمه تأخیر در تجهیز کارگاه"
                    className="h-10 text-sm text-right"
                    required
                  />
                </Field>

                <Field label="ماهیت/نوع جریمه" required>
                  <select
                    value={form.nature}
                    onChange={(e) => setForm(prev => ({ ...prev, nature: e.target.value }))}
                    className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                  >
                    {PENALTY_NATURES.map((opt) => (
                ...
                <Field label="روش محاسبه جریمه" required>
                  <select
                ...
                {isPercentMethod ? (
                  <Field label="درصد جریمه (%)" required>
                    <Input
                      type="number"
                      step="0.001"
                      value={form.rate}
                  ...
                  <Field label="مبلغ جریمه (ریال)" required>
                    <Input
                      type="number"
                      value={form.amount}
                ...
                <Table>
                  <TableHeader className="bg-muted/40">
                    <TableRow>
                      ...
                      <TableHead className="w-[30%] text-right text-xs font-bold text-foreground">عنوان نرخ جریمه</TableHead>
                      <TableHead className="w-[18%] text-center text-xs font-bold text-foreground">ماهیت جریمه</TableHead>
                      <TableHead className="w-[14%] text-center text-xs font-bold text-foreground">روش محاسبه</TableHead>
                      <TableHead className="w-[18%] text-center text-xs font-bold text-foreground">نرخ / مبلغ جریمه</TableHead>
                  ...
                  <TableBody>
                    {loading ? (
                      ...
                      filtered.map((item, idx) => {
                        const isPercent = item.calcMethod.includes("درصدی");
                        return (
                          <TableRow
                            key={item._id}
                          ...
                          >
                            ...
                            <TableCell className="text-center text-xs text-[10px] text-muted-foreground">{item.calcMethod}</TableCell>
                            <TableCell className="text-center text-xs font-mono font-bold">
                              {isPercent ? (
                                <span className="text-blue-600">{item.rate || 0}%</span>
                              ) : (
                                <span className="text-foreground">{formatNumber(item.amount)} ریال</span>
                              )}
                            </TableCell>
