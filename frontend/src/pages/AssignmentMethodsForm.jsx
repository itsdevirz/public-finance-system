...
const ASSIGNMENT_NATURES = [
  { value: "مناقصه", label: "مناقصه (Tender)" },
  { value: "مزایده", label: "مزایده (Auction)" },
  { value: "ترک تشریفات", label: "ترک تشریفات (Waiver)" },
  { value: "استعلام", label: "استعلام بها (Quotation)" },
  { value: "خرید مستقیم", label: "خرید مستقیم (Direct Purchase)" },
...
export default function AssignmentMethodsForm() {
  ...
  const fetchList = async () => {
    setLoading(true);
    try {
      const res = await api.get("/api/assignment-methods");
      if (res.data?.success) {
  ...
  const getSuggestedCode = async () => {
    try {
      const res = await api.get("/api/assignment-methods/suggest-code");
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (selectedId) {
        // Edit mode
        const res = await api.put(`/api/assignment-methods/${selectedId}`, payload);
        if (res.data?.success) {
          setSaved(true);
          fetchList();
        ...
        // Create mode
        const res = await api.post("/api/assignment-methods", payload);
        if (res.data?.success) {
          setSaved(true);
          fetchList();
          handleNew();
      ...
      console.error("Error saving assignment method:", err);
  ...
  const handleDelete = async () => {
    ...
    if (!confirm("آیا از حذف این روش واگذاری مطمئن هستید؟")) return;

    try {
      const res = await api.delete(`/api/assignment-methods/${selectedId}`);
      if (res.data?.success) {
        alert("روش واگذاری با موفقیت حذف شد.");
        fetchList();
        handleNew();
      }
    } catch (err) {
      console.error("Error deleting assignment method:", err);
      alert(err.response?.data?.message || "خطا در حذف روش واگذاری.");
    }
  };
  ...
  const handleRowClick = (item) => {
    ...
    setForm({
      code: item.code || "",
      title: item.title || "",
      nature: item.nature || "مناقصه",
      hasTenderCommittee: !!item.hasTenderCommittee,
      minAmount: item.minAmount !== undefined ? String(item.minAmount) : "",
      maxAmount: item.maxAmount !== undefined ? String(item.maxAmount) : "",
  ...
  const formatNumber = (num) => {
    if (!num && num !== 0) return "تعیین نشده";
    ...
    <PageShell>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-border/80 pb-4 mb-5" dir="rtl">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-lg font-bold text-foreground">تعریف روش واگذاری</h1>
            <p className="text-xs text-muted-foreground">مدیریت و تعریف روش‌های واگذاری، ارجاع کار و برگزاری مناقصات و مزایدات سازمان</p>
          </div>
        </div>
          ...
          <Card className="border-border/80 shadow-sm">
            ...
            <CardContent className="pt-5 pb-6">
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <Field label="کد روش واگذاری" required>
                ...
                <Field label="عنوان روش واگذاری" required>
                  <Input
                    type="text"
                    value={form.title}
                    onChange={(e) => setForm(prev => ({ ...prev, title: e.target.value }))}
                    placeholder="مثال: مناقصه عمومی یک مرحله‌ای"
                    className="h-10 text-sm text-right"
                    required
                  />
                </Field>

                <Field label="ماهیت/نوع واگذاری" required>
                  <select
                    value={form.nature}
                    onChange={(e) => setForm(prev => ({ ...prev, nature: e.target.value }))}
                    className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                  >
                    {ASSIGNMENT_NATURES.map((opt) => (
                ...
                <div className="flex items-center justify-between border border-border/80 rounded-md p-3 bg-muted/10">
                  <Label htmlFor="hasTenderCommittee" className="text-xs font-semibold text-muted-foreground cursor-pointer">
                    نیاز به تشکیل کمیسیون معاملات دارد
                  </Label>
                  <input
                    id="hasTenderCommittee"
                    type="checkbox"
                    checked={form.hasTenderCommittee}
                    onChange={(e) => setForm(prev => ({ ...prev, hasTenderCommittee: e.target.checked }))}
                    className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer"
                  />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <Field label="حداقل مبلغ معامله (ریال)">
                    <Input
                      type="number"
                  ...
                  <Field label="حداکثر مبلغ معامله (ریال)">
                    <Input
                      type="number"
                ...
                <Table>
                  <TableHeader className="bg-muted/40">
                    <TableRow>
                      ...
                      <TableHead className="w-[30%] text-right text-xs font-bold text-foreground">عنوان روش واگذاری</TableHead>
                      <TableHead className="w-[15%] text-center text-xs font-bold text-foreground">ماهیت واگذاری</TableHead>
                      <TableHead className="w-[10%] text-center text-xs font-bold text-foreground">کمیسیون</TableHead>
                      <TableHead className="w-[15%] text-center text-xs font-bold text-foreground">بازه مبالغ (ریال)</TableHead>
                      ...
                      filtered.map((item, idx) => (
                        ...
                        >
                          ...
                          <TableCell className="text-center text-xs text-muted-foreground">
                            {ASSIGNMENT_NATURES.find((n) => n.value === item.nature)?.label || item.nature}
                          </TableCell>
                          <TableCell className="text-center text-xs">
                            {item.hasTenderCommittee ? (
                          ...
                          <TableCell className="text-center text-xs font-mono text-[10px]">
                            {item.minAmount === undefined && item.maxAmount === undefined ? (
                              <span>نامحدود</span>
                            ) : (
                              <div>
                                {item.minAmount !== undefined && <div>از: {formatNumber(item.minAmount)}</div>}
                                {item.maxAmount !== undefined && <div>تا: {formatNumber(item.maxAmount)}</div>}
                              </div>
                            )}
                          </TableCell>
                          <TableCell className="text-center text-xs">
